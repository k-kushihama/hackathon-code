...
void loop() {
    ...
    // Ideal-time scheduling: avoid
  //cumulative drift caused by
    // child 1 (canon) blocking sendNEC +
 // waitGap each cycle.
    for (int c = 0; c < 2; c++) {
      if (now - lastTickMs[c] >=
  lastNoteDuration[c]) {
  ...
  lastNoteDuration[c];
        lastNoteDuration[c] =
  noteIntervalMs(idx[c]);
        idx[c]++;
        if (idx[c] >= melodyLength) idx[c]
  = 0;
        tickCount++;
        digitalWrite(LED_INDICATOR,
  (tickCount % 2) ? HIGH : LOW);
      }
    }

    if (now - lastTickMs[DRUM_CHILD_ID] >=
  lastNoteDuration[DRUM_CHILD_ID]) {
      sendDrum();
      lastTickMs[DRUM_CHILD_ID] +=
  lastNoteDuration[DRUM_CHILD_ID];
      lastNoteDuration[DRUM_CHILD_ID] =
  quarterMs();
      idx[DRUM_CHILD_ID]++;
