...
\begin{tikzpicture}[
  every node/.style={draw, rounded corners=2pt, align=center, font=\small,
                     minimum height=1.8em, inner sep=3pt},
...
\node {オーケストラシステム}
  child { node {Processing}
    child { node[text width=3.2cm] {音色・エフェクト\\付与関数} }
    child { node[text width=2.6cm] {音声再生関数} }
    child { node[text width=3.2cm] {演奏側との\\通信関数}
      child { node[text width=3.6cm, font=\scriptsize] {楽器音on/off切替用\\赤外線管理プログラム} }
    }
  }
  child { node[text width=2.8cm] {Arduino\\（親機）}
    child { node[text width=3cm] {演奏側との\\同期プロトコル}
      child { node[text width=3.4cm, font=\scriptsize] {楽譜進行用\\赤外線管理プログラム} }
    }
    child { node[text width=2.6cm] {同期用\\ハードウェア}
      child { node[font=\scriptsize] {赤外線ライト} }
    }
  }
  child { node[text width=2.8cm] {Arduino\\（子機）}
    child { node[text width=2.4cm] {楽譜情報}
      child { node[text width=2.8cm, font=\scriptsize] {音符データ\\（高さ・強さ・長さ）} }
      child { node[font=\scriptsize] {音符配列} }
    }
    child { node[text width=3cm] {Processingとの\\通信関数} }
    child { node[text width=2.8cm] {指揮側との\\同期プロトコル}
      child { node[text width=2.8cm, font=\scriptsize] {楽譜進行\\プログラム} }
      child { node[text width=2.8cm, font=\scriptsize] {楽器音管理\\プログラム} }
    }
    child { node[text width=2.4cm] {同期用\\ハードウェア}
      child { node[font=\scriptsize] {IRセンサ} }
    }
    child { node[text width=2.4cm] {LCD表示関数}
      child { node[text width=2.8cm, font=\scriptsize] {かえるの\\アニメーション\\表示プログラム} }
      child { node[text width=2.4cm, font=\scriptsize] {歌詞表示用\\プログラム} }
    }
  };
\end{tikzpicture}%
}% resizebox
...
もう1つの課題は発音遅延である．上記の合成をすべて発音のたびに計算すると，波形生成に時間がかかり演奏に間に合わないおそれがある．そこで，楽譜に登場する6種類の音高と2種類の音長の組をProcessingの起動時にすべて波形として生成し，MinimのSamplerにキャッシュしておく方式にした．演奏中はキャッシュ済みSamplerのtrigger()を呼ぶだけなので，発音時の処理は音源データの再生と同じで済む．Samplerは同時6声まで発音できる設定とし，前の音の余韻と次の音が自然に重なるようにした．未登録の音高が届いた場合はその場で生成してキャッシュに加える安全網も設けている．なお，このピアノ音源は当初オルガン風の持続音で仮実装しており，第9週に本節で述べた合成方式へ差し替えた．差し替え時には楽譜で使う全音高について試聴による検証を行い，全ケースで楽器音として問題ないことを確認している．波形生成部のコードは付録\ref{sec:code-pde}に示す．
...
LCD表示と演奏の同期に関する評価結果を表\ref{tab:lcd-result}に示す．同期誤差は目標100\,ms以内に対して約50.1\,msであり，十分な精度で同期できている．
...
表\ref{tab:overall-result}に評価分類ごとの総合判定を示す．同期・通信制御では赤外線通信誤差平均28\,ms（目標30\,ms以内），受信率100\,\%（目標95\,\%以上）と全項目で目標を達成した．LCD表示同期も約50.1\,msと目標100\,ms以内を達成した．音色表現は過半数の評価者から良好と判定されたが，トランペットとキックの評価平均が3.6台にとどまっており，改善の余地がある．
...
7月13日に実施した定量評価により，演奏タイミング誤差平均28\,ms（目標30\,ms以内），信号受信率10000回中10000回成功（100\,\%），LCD同期誤差約50.1\,ms（目標100\,ms以内）と主要な到達目標を全て達成した．同期・通信・LCD表示は全項目で目標を達成し，音色表現も過半数の評価者から良好と判定された．Sony SIRC 12ビットの採用によりフレーム長を最小化しつつ，屋内環境で十分な通信品質を確保でき，赤外線通信による輪唱システムは有効であることが実証された．
