...
第10週に実施した定量計測の結果を以下に示す．

\subsubsection{同期・通信の評価結果}

赤外線通信による同期の定量評価結果を表\ref{tab:sync-result}に示す．演奏タイミング誤差は目標30\,ms以内に対して平均28\,msで達成，信号受信率は100回中100回の成功で100\,\%を記録し，テンポ変更への追従も1拍以内に収まった．
...
評価項目 & 目標 & 結果 & 判定 \\
\hline
赤外線通信誤差 & 平均30\,ms以内 & 平均28\,ms & 達成 \\
信号受信率 & 95\,\%以上 & 100\,\%（100回中100回成功） & 達成 \\
...
表\ref{tab:overall-result}に評価分類ごとの総合判定を示す．同期・通信制御とLCD表示同期は全項目で目標を達成した．音色表現は過半数の評価者から良好と判定されたが，トランペットとキックの評価平均が3.6台にとどまっており，改善の余地がある．
...
LCD表示同期 & 同期誤差 約50\,ms & 達成 \\
総合評価 & 主要目標達成，音色表現に改善余地 & 一部改善が必要 \\
\hline
\end{tabular}
\end{center}
\end{table}
...
第10週の定量計測により，いくつかの新たな知見が得られた．

赤外線通信方式をIR\_SONY（Sony SIRC 12bit）に変更したことで，演奏タイミング誤差の目標値30\,msを平均28\,msで達成できた．送受信距離が適切に保たれていれば通信エラーは発生せず，100回の試行で受信率100\,\%を記録した．演奏側が毎拍ごとに同期信号を受信する設計により，テンポ変更にも素早く追従する動作が確認された．
...
定量評価により，演奏タイミング誤差平均28\,ms，信号受信率100\,\%など主要な到達目標を測定により確認した．同期・通信・LCD表示は全項目で目標を達成し，音色表現も過半数の評価者から良好と判定された．赤外線通信による輪唱システムは有効であることが実証された．
...
7/7 & 計測 & 同期・通信の定量評価実施（誤差28\,ms，受信率100\,\%） \\
7/7 & 計測 & 音色アンケート実施（18人，5段階評価） \\
7/7 & 計測 & LCD同期誤差の計測（約50.1\,ms） \\
7/9 & 文書 & 報告書を提出 \\
\hline
\end{tabular}
\end{center}
