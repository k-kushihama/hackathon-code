...
プロジェクト名 & 赤外線通信で歌うカエル\\
\hline
担当パート & 親機と子機の赤外線通信（輪唱機能作成）：親機ファーム hack-oya.ino，子機ファーム hack-ko.ino，両者を媒介する Processing 側 hackko.pde の休符処理，および親機回路（ボタン・LED・start ボタン）の組み立て\\
\hline
本来の今週の位置づけ & 第8週末（6/23）．計画書 v1.2 のガント上，第8週は 510:輪唱機能作成（全員）の実施期間．櫛濱は親機⇔子機の赤外線通信プロトコル（NEC ブロードキャスト方式）と，子機側で受信した cyclePos／enabledMask から localPos を逆算して発音する経路の整備を担当\\
\hline
報告対象 & 前回作業ログ（6/16）以降に実施した\textbf{親機と子機の赤外線通信による輪唱機能の実機完成}．具体的には親機回路の組み立て（子機ボタン D4--D7／状態表示 LED A0--A3／start ボタン D8），親機ファーム（hack-oya.ino）の押下順カノン・誤操作ガード・チャタリング対策，子機ファーム（hack-ko.ino）の親機との TICK\_LENGTH 整合，Processing（hackko.pde）の休符即時停止\\
\hline
作業時間 & 計10時間\\
\hline
\end{tabular}
...
\begin{itemize}
    \item 現在地：第8週末（6/23）．前回ログ（6/16）で持ち越していた「複数子機での輪唱実機実験」を実施し，\textbf{親機⇔4 子機（メロディ 3＋ドラム 1）の赤外線通信による輪唱演奏が成立}した
...
§2.1 のチーム全体ガントから櫛濱担当分（櫛濱・久家／全員）を抜き出した個人計画．第8週は 510:輪唱機能作成（全員）の実施期間．本週は親機⇔子機の赤外線通信プロトコル（NEC ブロードキャスト）と発音経路の整備により 510 の実機完成を達成．
...
\begin{itemize}
    \item 現在地は第8週末（6/23）．GC 上，第8週は 510:輪唱機能作成（全員）の実施期間．前回ログ時点で機材不足のため物理的に実施できていなかった「親機⇔複数子機の赤外線通信による輪唱実機実験」を今週で完遂した．
    ...
    \item 次週以降は 520 結合テスト本体に入り，MOP5 のフェードアウト実装と MOP3（IR 通信成功率）の定量計測を進める．
\end{itemize}
...
\hline
MOP3 & 赤外線通信テスト & 暫定OK（複数子機） & 4 子機同時受信は確認．\textbf{成功率の定量計測は未実施}（第9週で対応） \\
\hline
...
\begin{itemize}
    \item MOP3：4 子機を一定時間動かして送受信ログから成功率を算出（第9週）
...
図~\ref{fig:system} に第8週末時点の輪唱システム全体を示す．親機 1 台（hack-oya.ino）が 4 子機（hack-ko.ino，CHILD\_ID=0〜3）へ NEC フレームを一斉送出し，各子機は自分宛 mask が立った tick で localPos を計算し，対応する楽譜要素を CSV で Processing へ送る．child0〜2 はメロディ機（hackko.pde で発音），child3 はドラム機（drum.pde で発音）．
...
\subsubsection*{9.3.2 親機 hack-oya の回路図}
...
\subsubsection*{9.3.3 親機 hack-oya の回路実装イメージ}
