...
プロジェクト名 & 赤外線通信で歌うカエル\\
\hline
担当パート & Processing（ピアノ演奏プログラム）／Arduino（ドラム子機）\\
\hline
本来の今週の位置づけ & 第7週末（子機プログラム実装週）\\
\hline
今週の主担当 & 210：ピアノのプログラム（Processing 連携）＋ ドラム子機 \texttt{.ino}（180 を久家と分担）\\
\hline
...
\begin{itemize}
    ...
    \item 今週は \textbf{210：ピアノのプログラム（Processing 連携）} と \textbf{ドラム子機の Arduino プログラム（\texttt{hack-drum.ino}）} を実装し，\textbf{いずれも実機で動作確認まで完了}
    \item 親機（\texttt{hack-oya2.ino}）と統合し，\textbf{輪唱（カノン）＋ドラム伴奏が同期して鳴る}ところまで通った
    ...
    \begin{itemize}
        ...
        \item ドラム子機（\texttt{hack-drum.ino}）が NEC フレーム（\texttt{address==2}）のみを拾い，\texttt{command}（MIDI 番号）をシリアルへ中継．自前タイマを持たず\textbf{テンポは親機任せ}で可変 BPM に追従
        \item 通信経路「親機 → 赤外線（NEC）→ 子機 → USB シリアル → Processing → 発音」が一気通貫で動作
    \end{itemize}
    \item 重大課題（影響度：高）：
    \begin{itemize}
...
第7週で 210（ピアノのプログラム）を完了．加えて，ドラム担当（久家）の負荷分散として\textbf{ドラム子機の \texttt{.ino} を新たに分担}した（橙色）．Processing 側のシリアル受信処理を実装した関係で，120（通信用関数）の一部も第7週に前倒しで着手済み．
...
\hline
180 & 100 (Processing) & ドラム子機 \texttt{.ino}（久家と分担）\textsuperscript{追加} & & & \cellcolor{ganttEarly} & & & \\
\hline
...
\begin{itemize}
    ...
    \item さらに，ドラム子機の Arduino プログラム（\texttt{hack-drum.ino}）を分担し，親機統合で輪唱＋ドラムの同期発音まで到達．
    \item 次週（第8週）は 120（通信用関数）の正式化と MOP5（フェードアウト）対応に移る．
\end{itemize}
...
\hline
ドラム子機 \texttt{.ino}（180 分担） & 6/4 & 6/9 & 完了 & 100\％ & 親機（\texttt{hack-oya2.ino}）の NEC 送出 & NEC（\texttt{address==2}）の \texttt{command} をシリアルへ中継．自前タイマなしでテンポは親機任せ（付録 9.3.2） \\
\hline
親機統合・動作確認 & 6/9 & 6/9 & 完了 & 100\％ & 全子機 \texttt{.ino}／Processing & 親機 → IR → 子機 → Processing の経路で輪唱＋ドラムの同期発音を確認（付録 9.3.3） \\
\hline
\end{longtable}
...
計画書 v1.2「1.5 テスト項目（修正版）」のうち，今週の実装（210 ピアノのプログラム，ドラム子機 \texttt{.ino}，通信経路）に対応する項目を整理する．
...
\hline
MOP2 & シリアル通信テスト & OK & 921600bps で子機 → Processing の MIDI 受信．取りこぼしなく発音 \\
\hline
MOP3 & 赤外線通信テスト & 暫定OK & 親機 → ドラム子機（NEC \texttt{address==2}）の受信成功を確認．成功率の定量計測は未実施 \\
\hline
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item 親機（\texttt{hack-oya2.ino}）と接続し，IR 経由で届く番号で発音されるか実機確認
    \end{itemize}
    \item 参照資料：
    \begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item ピアノ子機・ドラム子機がそれぞれ MIDI 番号をシリアルへ中継し，Processing が発音するかを聴感と波形で確認
    \end{enumerate}
\end{itemize}
...
\begin{itemize}
    ...
    \item 根拠：
    \begin{itemize}
        \item 親機→IR→子機→Processing の経路で輪唱＋ドラムが同期発音し，子機プログラムの目的を満たしているため
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item ドラム子機を分担したため，久家とドラムパターン（\texttt{drumPattern[]}）の最終仕様をすり合わせ予定
...
\begin{itemize}
    \item ソースコード：\texttt{hack\_ko2.pde}（ピアノ Processing プログラム），\texttt{hack-drum.ino}（ドラム子機 Arduino）
...
\begin{itemize}
    ...
    \item 分担したドラム子機 Arduino のソース，および通信経路の構成図・回路図
\end{itemize}
...
\subsubsection*{9.3.2 ドラム子機 Arduino プログラム（\texttt{hack-drum.ino}）}

赤外線受光モジュールで NEC フレームを受信し，自分宛（\texttt{CHILD\_ID = 2}）のものだけを拾って \texttt{command}（MIDI 番号）をシリアルへ中継する．楽譜を持たずステートレスで，テンポはすべて親機任せのため可変 BPM に自動追従する．
...
図~\ref{fig:dataflow} に，親機から発音までのデータフローを示す．親機（\texttt{hack-oya2.ino}）が \texttt{melody[]}／\texttt{drumPattern[]} に沿って NEC フレーム（\texttt{address}＝子機 ID，\texttt{command}＝MIDI 番号）を送出し，各子機は自分宛のフレームだけを拾って MIDI 番号をシリアルへ中継，Processing が受信して発音する．ピアノ子機は \texttt{CANON\_OFFSET\_TICKS} 分だけ遅れて開始することで輪唱（カノン）を実現する．
