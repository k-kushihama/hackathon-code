...
プロジェクト名 & 赤外線通信で歌うカエル\\
\hline
担当パート & Processing（ピアノ演奏プログラム）／Arduino（ドラムの通信用関数）\\
\hline
本来の今週の位置づけ & 第7週末（子機プログラム実装週）\\
\hline
今週の主担当 & 210：ピアノのプログラム（Processing 連携）＋ 180：ドラムの通信用関数 \texttt{hack-drum.ino}（ドラム音色 170/171 は久家担当の別タスク）\\
\hline
補足 & 第6週はログ未提出のため，本ログは第5週末ログ（5/27）からの続きとして記載\\
\hline
\end{tabular}
...
\begin{itemize}
    \item 現在地：第7週末（6/9）．実装フェーズの子機プログラム週．第6週はログ未提出だが，第5週末に先行着手していた 110（ピアノ音の作成）はその後完了済み
    \item 今週は \textbf{210：ピアノのプログラム（Processing 連携）} と \textbf{180：ドラムの通信用関数（\texttt{hack-drum.ino}）} を実装．210 は\textbf{実機で動作確認まで完了}したが，180 は\textbf{プログラム作成のみで実機テストは未実施}
    \item 第6週は対面できなかったため検証範囲が限られるが，ピアノ系（親機 → 赤外線 → ピアノ子機 → Processing）では \texttt{hack-oya2.ino} と \texttt{hack\_ko2.pde} を用いて\textbf{親機からの譜面送信と発音に成功}
    \item ドラムは久家と作業を分け，今回は \textbf{180：ドラムの通信用関数（\texttt{hack-drum.ino}）}を櫛濱が担当（ドラム音色 170/171 の \texttt{.pde} 作成は久家担当の別タスク）．本通信用関数は\textbf{プログラムを作成したのみで実機テストは未実施}
    ...
    \begin{itemize}
        ...
        \item ドラムの通信用関数（\texttt{hack-drum.ino}，課題 180）は NEC フレーム（\texttt{address==2}）のみを拾い \texttt{command}（MIDI 番号）をシリアルへ中継する設計で実装（自前タイマを持たず\textbf{テンポは親機任せ}）．久家のドラム音色（\texttt{.pde}）と組み合わせて鳴らす想定だが，\textbf{実機テストは未実施}
    \end{itemize}
    \item 重大課題（影響度：高）：
    \begin{itemize}
        \item ドラムの通信用関数（\texttt{hack-drum.ino}，180）が未テスト：第6週に対面できず，久家のドラム音色（\texttt{.pde}）と結合した実機での受信・発音確認ができていない
...
計画書 v1.2（2026/5/29 版）でチーム全体のスケジュールが更新されたため，チーム全体・個人の両方について変更前後を併記する．橙色セル（\colorbox{ganttEarly}{\phantom{xx}}）は\textbf{前倒し・先行着手}した週，水色セル（\colorbox{ganttCell}{\phantom{xx}}）は計画通りの実施週を表す．
...
\subsection*{2.2 チーム全体ガントチャート（変更後：計画書 v1.2・第7週末時点）}

計画書 v1.2 で 5 週列を追加し，先行着手・前倒し分を反映した．171（ドラムシンバル）は新規追加タスク．

\begin{longtable}{|p{0.9cm}|p{2.0cm}|p{4.0cm}|p{1.5cm}|c|c|c|c|c|c|}
\hline
\rowcolor[gray]{0.9}
管理 & 第2層 & 第3層タスク & 担当者 & 5週 & 6週 & 7週 & 8週 & 9週 & 10週 \\
\hline
110 & 100 (Processing) & ピアノ音の作成 & 櫛濱和輝 & \cellcolor{ganttEarly} & \cellcolor{ganttCell} & & & & \\
\hline
120 & 100 (Processing) & arduinoとの通信用関数（ピアノ） & 櫛濱和輝 & & & & \cellcolor{ganttCell} & \cellcolor{ganttCell} & \\
\hline
130 & 100 (Processing) & フルート音の作成 & 家田祐希 & \cellcolor{ganttEarly} & & & & & \\
\hline
140 & 100 (Processing) & arduinoとの通信用関数（フルート） & 家田祐希 & & & & \cellcolor{ganttCell} & \cellcolor{ganttCell} & \\
\hline
150 & 100 (Processing) & トランペット音の作成 & 木下遼介 & \cellcolor{ganttEarly} & & & & & \\
\hline
160 & 100 (Processing) & arduinoとの通信用関数（トランペット） & 木下遼介 & \cellcolor{ganttEarly} & & & \cellcolor{ganttCell} & \cellcolor{ganttCell} & \\
\hline
170 & 100 (Processing) & ドラム音の作成（キック） & 久家力孔 & & \cellcolor{ganttCell} & & & & \\
\hline
171 & 100 (Processing) & ドラム音の作成（シンバル）\textsuperscript{新規} & 久家力孔 & & \cellcolor{ganttCell} & & & & \\
\hline
180 & 100 (Processing) & arduinoとの通信用関数（ドラム） & 久家力孔 & & & & \cellcolor{ganttCell} & \cellcolor{ganttCell} & \\
\hline
210 & 200 (子機) & ピアノのプログラム & 櫛濱和輝 & & & \cellcolor{ganttCell} & & & \\
...
220 & 200 (子機) & フルートのプログラム & 家田祐希 & & & \cellcolor{ganttCell} & & & \\
\hline
230 & 200 (子機) & トランペットのプログラム & 木下遼介 & \cellcolor{ganttEarly} & & \cellcolor{ganttCell} & & & \\
\hline
240 & 200 (子機) & 楽譜の設定 & 渡邊乃斗 & & \cellcolor{ganttCell} & & & & \\
\hline
310 & 300 (親機) & 赤外線管理プログラム & 渡邊乃斗 & & & \cellcolor{ganttCell} & & & \\
...
\textbf{主な変更点（計画書 v1.1 → v1.2）：}
\begin{itemize}
    \item 110（ピアノ音の作成）：第6週予定 → 第5週末に先行着手（その後完了）
    \item 130（フルート音）・150（トランペット音）：第6週予定 → 第5週末に前倒し完了
    \item 160（トランペット通信用関数）・230（トランペット子機プログラム）：第7週予定 → 第5週末から先行着手
    \item 171（ドラム音の作成・シンバル）：新規追加タスク
    \item 130 の担当変更（渡邊 → 家田）
\end{itemize}
...
第7週で 210（ピアノのプログラム）を完了．加えて，ドラムは久家と作業を分け，\textbf{180：ドラムの通信用関数（\texttt{hack-drum.ino}）を櫛濱が担当}（橙色．音色 170/171 の \texttt{.pde} は久家の別タスク）．Processing 側のシリアル受信処理を実装した関係で，120（通信用関数・ピアノ）の一部も第7週に前倒しで着手済み．ただし 180 は第6週に対面できず\textbf{実機テストは未実施}（プログラム作成のみ）．
...
管理 & 第2層 & 第3層タスク & 5週 & 6週 & 7週 & 8週 & 9週 & 10週 \\
\hline
110 & 100 (Processing) & ピアノ音の作成（完了） & \cellcolor{ganttCell} & \cellcolor{ganttCell} & & & & \\
\hline
210 & 200 (子機) & ピアノのプログラム（Processing 連携，完了） & & & \cellcolor{ganttCell} & & & \\
\hline
180 & 100 (Processing) & ドラムの通信用関数 \texttt{hack-drum.ino}（音色は久家／未検証）\textsuperscript{追加} & & & \cellcolor{ganttEarly} & & & \\
\hline
120 & 100 (Processing) & arduinoとの通信用関数（ピアノ） & & & \cellcolor{ganttEarly} & \cellcolor{ganttCell} & \cellcolor{ganttCell} & \\
\hline
510 & 500 (包括) & 結合テスト（全員） & & & & & & \cellcolor{ganttCell} \\
\hline
\end{longtable}
...
\begin{itemize}
    \item 現在地は第7週末（6/9）．本来は子機プログラム（210/220/230）の実装週．
    \item 予定どおり \textbf{210：ピアノのプログラム}（Processing 側でシリアル受信→発音）を実装・動作確認まで完了．
    \item ピアノは親機 → 赤外線 → ピアノ子機 → Processing の経路で\textbf{譜面送信・発音に成功}．
    \item ドラムは久家と作業を分け，櫛濱が 180：ドラムの通信用関数（\texttt{hack-drum.ino}）を実装したが，第6週に対面できず\textbf{実機テストは未実施}（音色 170/171 は久家担当）．
    \item 次週（第8週）はドラム通信用関数の実機テスト，120（通信用関数・ピアノ）の正式化，MOP5（フェードアウト）対応に移る．
\end{itemize}
...
\hline
ピアノのプログラム (210) & 6/3 & 6/9 & 完了 & 100\％ & 子機 Arduino（\texttt{hack-ko2.ino}）／親機 & Processing でシリアル受信した MIDI 番号を周波数へ変換し \texttt{PianoInstrument} で発音．波形描画も実装（付録 9.3.1） \\
\hline
ドラムの通信用関数 (180) \texttt{hack-drum.ino} & 6/4 & 6/9 & 作成済（未検証） & 実装100\％／検証0\％ & 親機の NEC 送出／久家のドラム音色 \texttt{.pde} & NEC（\texttt{address==2}）の \texttt{command} をシリアルへ中継する設計で作成．第6週に対面できず実機テスト未実施（付録 9.3.2） \\
\hline
ピアノ系の統合・動作確認 & 6/9 & 6/9 & 完了 & 100\％ & ピアノ子機 \texttt{hack-ko2.ino}／Processing & 親機 → IR → ピアノ子機 → Processing の経路で譜面送信・発音を実機確認．ドラムは未テスト（付録 9.3.3） \\
\hline
\end{longtable}
...
計画書 v1.2「1.5 テスト項目（修正版）」のうち，今週の実装（210 ピアノのプログラム，180 ドラムの通信用関数，通信経路）に対応する項目を整理する．
...
\hline
MOP3 & 赤外線通信テスト & 暫定OK & 親機 → ピアノ子機経由で譜面を送信し発音を確認．ドラムの通信用関数は未テスト．成功率の定量計測も未実施 \\
\hline
MOP5 & フェードアウト & 未対応 & \texttt{noteOff()} が即時 \texttt{unpatch} のため切り替えノイズの懸念．次週対応 \\
\hline
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item ピアノ子機（\texttt{hack-ko2.ino}）と親機（\texttt{hack-oya2.ino}）を接続し，IR 経由で届く譜面で発音されるか実機確認（ドラムの通信用関数は未テスト）
    \end{itemize}
    \item 参照資料：
    \begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item ピアノ子機が MIDI 番号をシリアルへ中継し，Processing が発音するかを聴感と波形で確認（ドラムの通信用関数は同手順で次週テスト予定）
    \end{enumerate}
\end{itemize}
...
\begin{itemize}
    ...
    \item 根拠：
    \begin{itemize}
        \item 親機 → IR → ピアノ子機 → Processing の経路で譜面送信・発音まで確認でき，ピアノ系の目的を満たしているため（ドラムの通信用関数は次週テスト予定）
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item 目標（第8週）：
    \begin{itemize}
        \item ドラムの通信用関数（\texttt{hack-drum.ino}，180）の実機テスト：親機 → ドラム子機 → Processing で，久家のドラム音色（\texttt{.pde}）と結合して受信・発音を確認
...
\begin{itemize}
    \item ドラムは通信用関数（櫛濱・180）と音色（久家・170/171）で担当が分かれるため，\texttt{drumPattern[]} の最終仕様と \texttt{.pde} 側の発音をすり合わせ予定
...
\begin{itemize}
    \item ソースコード：\texttt{hack\_ko2.pde}（ピアノ Processing プログラム），\texttt{hack-drum.ino}（ドラムの通信用関数・180）
...
\begin{itemize}
    ...
    \item 担当したドラムの通信用関数（\texttt{hack-drum.ino}）のソース，および通信経路の構成図・回路図
\end{itemize}
...
\subsubsection*{9.3.2 ドラムの通信用関数（\texttt{hack-drum.ino}・課題 180）}

ドラムは作業を分担し，本プログラム（通信用関数）を櫛濱が，ドラム音色の \texttt{.pde}（170/171）を久家が担当する．赤外線受光モジュールで NEC フレームを受信し，自分宛（\texttt{CHILD\_ID = 2}）のものだけを拾って \texttt{command}（MIDI 番号）をシリアルへ中継する．楽譜を持たずステートレスで，テンポはすべて親機任せのため可変 BPM に自動追従する設計だが，第6週に対面できず\textbf{実機テストは未実施}．
