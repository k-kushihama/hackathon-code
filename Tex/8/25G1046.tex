...
\hline
本来の今週の位置づけ & 第8週末（6/16）．計画書 v1.2 のガント上，第8週（6/10〜6/16）は櫛濱の割当タスクが直接は載っていないが，輪唱実験（実機統合）と通信仕様の見直しを実施した期間\\
\hline
...
\subsection*{2.1 チーム全体ガントチャート（計画書 v1.2）}
...
計画書 v1.2 のチーム全体ガント（§2.1）から櫛濱担当分を抜き出した個人計画．音色作成（110）・通信用関数（120）を第5〜6週，ピアノのプログラム（210）を第5週，ドラムのプログラム（240）を第6週に置く．第8週は GC 上では櫛濱の割当タスクは載っていないが，実機統合（輪唱実験）と通信仕様見直しを行った期間．
...
\hline
240 & 200 (子機) & ドラムのプログラム（hack-ko.ino／drum.pde） & & \cellcolor{ganttCell} & & & & \\
\hline
...
\begin{itemize}
    \item 現在地は第8週末（6/16）．GC 上，第8週（6/10〜6/16）に櫛濱の割当タスクは載っていないが，第7週末ログで残課題としていた「ドラムのプログラム（240）の実機テスト」と「通信仕様の整理」を実施した．
    \item 通信仕様について，音価・音量の決定主体を Processing 側から Arduino 側へ移し，子機 .ino を「音楽情報の源泉」，Processing を「再生エンジン」と役割分担し直した（CSV 経由の片方向通知）．
...
図~\ref{fig:circuit} にドラム子機 Arduino の接続を示す．赤外線受光モジュール CHQ1838D を 1 つ用い，親機からの NEC フレームを受信する．D2 に受光信号，D13 に動作確認用 LED を接続し，USB を Processing 実行用 PC へつなぐ．6/13 のドラム実機テストではこの構成を Arduino UNO R3（追加購入分）で組み，自 Mac の drum.pde と接続した．
